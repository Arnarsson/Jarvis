...
must_haves:
  ...
  artifacts:
    ...
    - path: "server/alembic/versions/002_add_processing_status.py"
      provides: "Migration for processing_status and ocr_text columns"
...
<task type="auto">
  <name>Task 1: Add processing status and ocr_text to Capture model</name>
  <files>
server/src/jarvis_server/db/models.py
server/alembic/versions/002_add_processing_status.py
  </files>
  <action>
Update Capture model to track processing status and store OCR text.

**server/src/jarvis_server/db/models.py:**
Add new fields to Capture class:
```python
...
# OCR extracted text from the capture image
...
Create migration:
```python
"""Add processing_status and ocr_text to captures table.
...
```
  ...
  <verify>
Run `python -c "from jarvis_server.db.models import Capture; print(Capture.__table__.columns.keys())"` - should include processing_status and ocr_text.
Check migration file exists: `ls server/alembic/versions/002*`
  </verify>
  <done>Capture model has processing_status field (pending/processing/completed/failed) and ocr_text field with index for backlog queries.</done>
</task>
...
2. Model has fields: `python -c "from jarvis_server.db.models import Capture; cols = [c.name for c in Capture.__table__.columns]; print('processing_status' in cols and 'ocr_text' in cols)"`
...
- Capture model has ocr_text field for storing extracted text
